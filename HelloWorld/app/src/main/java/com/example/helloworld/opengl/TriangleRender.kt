...
class TriangleRender : GLSurfaceView.Renderer {
    ...
    override fun onSurfaceCreated(p0: GL10?, p1: EGLConfig?) {
        ...
        val shaderObjectId: Int = GLES20.glCreateShader(GLES20.GL_VERTEX_SHADER)
        GLES20.glShaderSource(shaderObjectId, vertexShaderCode)//loadShader(GLES20.GL_VERTEX_SHADER, vertexShaderCode)


        val shaderObject2: Int = GLES20.glCreateShader(GLES20.GL_FRAGMENT_SHADER)
        GLES20.glShaderSource(shaderObject2, fragmentShaderCode)

        mProgram = GLES20.glCreateProgram()
        // 将顶点着色器加入到程序
        GLES20.glAttachShader(mProgram!!, shaderObjectId)
        // 将片元着色器加入到程序中
        GLES20.glAttachShader(mProgram!!, shaderObject2)
